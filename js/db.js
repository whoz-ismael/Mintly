...
const DB = {
  ...
  async getAccounts(includeArchived = false) {
    const { data: { user } } = await supabase.auth.getUser();
    let query = supabase.from('accounts').select('*').eq('user_id', user.id).order('created_at');
  ...
  async updateAccountBalance(accountId, newBalance) {
    return supabase.from('accounts').update({ balance: newBalance }).eq('id', accountId);
  },
  ...
  async createTransfer(transfer) {
    ...
    if (!error) {
      ...
      const { data: fromAcc } = await supabase.from('accounts').select('balance').eq('id', transfer.from_account_id).single();
      const { data: toAcc }   = await supabase.from('accounts').select('balance').eq('id', transfer.to_account_id).single();
  ...
  async getInstallments(accountId) {
    let query = supabase.from('installment_plans').select('*').eq('is_active', true);
    if (accountId) query = query.eq('account_id', accountId);
  ...
  async createInstallment(plan) {
    const { data: { user } } = await supabase.auth.getUser();
    const { data, error } = await supabase.from('installment_plans')
  ...
  async getMoneyLent() {
    const { data: { user } } = await supabase.auth.getUser();
    const { data } = await supabase.from('money_lent')
  ...
  async createMoneyLent(item) {
    ...
    const { data, error } = await supabase.from('money_lent')
  ...
  async getBusinessIncome(businessId) {
    const { data } = await supabase.from('business_income')
  ...
  async addBusinessIncome(income) {
    ...
    const { data, error } = await supabase.from('business_income')
  ...
  async addGoalContribution(goalId, amount, currency) {
    ...
    if (goal) {
      ...
      await supabase.from('savings_goals').update({ current_amount: newAmount, is_completed: isCompleted }).eq('id', goalId);
    }
  },

  // ── CRÉDITO ──────────────────────────────────────────────
  async updateCreditLimit(accountId, newLimit, changeDate) {
    const { data: acc } = await supabase.from('accounts').select('credit_limit').eq('id', accountId).single();
    if (acc) {
