...
const DB = {
  ...
  async createTransfer(transfer) {
    ...
    if (!error) {
      ...
      const { data: fromAcc } = await sb.from('accounts').select('balance, type').eq('id', transfer.from_account_id).single();
      const { data: toAcc }   = await sb.from('accounts').select('balance, type').eq('id', transfer.to_account_id).single();

      if (fromAcc) {
        // La cuenta origen siempre pierde el monto (sea crédito o no)
        const fromDelta = fromAcc.type === 'credit'
          ? transfer.from_amount   // pagar desde crédito sube la deuda
          : -transfer.from_amount; // pagar desde cuenta normal baja el saldo
        await this.updateAccountBalance(transfer.from_account_id, fromAcc.balance + fromDelta);
      }
      if (toAcc) {
        // Si el destino es una tarjeta de crédito, el pago REDUCE la deuda
        const toDelta = toAcc.type === 'credit'
          ? -transfer.to_amount   // pagar a tarjeta reduce lo adeudado
          : transfer.to_amount;   // transferir a cuenta normal suma el saldo
        await this.updateAccountBalance(transfer.to_account_id, toAcc.balance + toDelta);
      }
    }
    return { data, error };
  },
